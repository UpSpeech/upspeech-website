...
// Size comes from the markup, not from the name. An article that declares
// .frame-page is 1080 x 1350 whatever it is called, so a LinkedIn photo post
// and a carousel page do not have to be named alike to render alike.
async function declaredSize(name) {
  const html = await fs.readFile(htmlPath, "utf8");
  const tag = html.match(
    new RegExp(`<article[^>]*data-export="${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"[^>]*>`),
  )?.[0];
  if (!tag) return null;
  if (/\bframe-page\b/.test(tag)) return { width: 1080, height: 1350 };
  if (/\bframe-story\b/.test(tag)) return { width: 1080, height: 1920 };
  if (/\bframe-square\b/.test(tag)) return { width: 1080, height: 1080 };
  return null;
}
...
for (const name of names) {
  const { width, height } = (await declaredSize(name)) ?? exportSize(name);
